...
public interface RiderDeliveryJoinRepository  extends JpaRepository<DeliverResponseWithOriginAddress, Integer>{
	...
	@Query(value = "select d.deliveryid,d.userid,d.pickuptime,d.dropaddress,d.droplatitude,d.droplongitude,d.status,d.riderid,d.orderid,d.deliverycharge,d.description,d.img,d.weightcategory,d.destinationcontact,d.isDelicate,d.isBalloonAdded,d.isBouqetAdded,d.isTwoCakes,d.originaddressid as originaddressid,d.paymentMethod as paymentMethod,d.locationcode as locationcode,a.housenumber as originhousenumber,a.floor as originfloor,a.block as originblock,a.latitude as originlat,a.longitude as originlong,a.locality as originlocality,a.city as origincity,a.addressid as addressid from Deliveries d LEFT JOIN Address a ON  d.originaddressid=a.addressid where d.status = :status And d.locationcode = :locationcode order by deliveryid desc",nativeQuery = true)
	public List<DeliverResponseWithOriginAddress> getJoinedInfo(@Param("status")String status,@Param("locationcode")int locationcode);
