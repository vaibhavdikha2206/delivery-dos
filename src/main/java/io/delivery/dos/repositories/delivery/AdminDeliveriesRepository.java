...
public interface AdminDeliveriesRepository extends JpaRepository<Deliveries, Integer>{
	...
	@Query(value="select * from Deliveries where pickuptime = :pickuptime And riderid is not null And locationcode = :locationcode order by deliveryid desc",nativeQuery = true)
	List<Deliveries> findByPickuptimeWhereRideridIsNotnull(@Param("pickuptime")String pickuptime,@Param("locationcode")int locationcode);
}
